...
long  permutation(unsigned long S_box)
{
	long p = 0;
	for (int i = 0; i < 32; i++)
	{
...
void Round(ll k, unsigned long& R, unsigned long& L)
{
	long r = R;
	ll temp = 0;
	temp = Expantion(R);
...
long LCS(unsigned long& d, unsigned long sh)
{
	long h = 0;
	for (int i = 0; i < 28; i++)
	{
...
void split_key(ll k, unsigned long& D, unsigned long& C)
{
	for (int i = 0; i < 28; i++)
	{
		C |= GET_BIT(k, i) << i;
...
ll concatenate(unsigned long D, unsigned long C)
{
	ll k = 0;
	for (int i = 0; i < 28; i++)
	{
...
ll generate_key_i(unsigned long& D, unsigned long& C, unsigned long i)
{
	ll key_i = 0;
...
void  generate_16_key(unsigned long& D, unsigned long& C)
{
	ll key_i = 0;
	for (int i = 1; i < 17; i++)
	{
...
int main()
{
	...
	ifstream f;
	ofstream o, p;
	unsigned char cnt = 0, choice = 0, read_char;

	/* Encrypte or Decrypte */
	printf("Enter 1 to Encrypte\n");
	printf("Enter 2 to Decrypte\n");
	cin >> choice;
	...
	/* get the input and write encrypted output as hex and as plain text */
	if (choice == '1')
	{
		f.open("plain.txt");
	...
	/* get the input and write decrypted output as plain text */
	else if (choice == '2')
	{
		f.open("hex.txt");
		p.open("decrypt.txt");
